...
app.post('/login-submit', function (request, response) {
  ...
  db.query('SELECT * FROM userss WHERE email = ?', [formData.email], function (error, results) {
    if (error) {
...
app.post('/register-submit', function (request, response) {
  const formData = request.body;
  console.log(formData);
  db.query('INSERT INTO userss(first_name,last_name,email,password,birth) VALUES (?, ?, ?, ?,?)', [formData.firstname, formData.lastname, formData.email, formData.password,formData.birth], function (error, results) {
    if (error) {
      response.status(500).send('Error in query')
      return
    }
